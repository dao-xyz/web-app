...
import { SignForMeAccount, createSignForMe, deleteSignForMeAsOwner, getSigningCapabilities, getDelegatedSigners } from "@dao-xyz/sdk-signforme";
...
interface ISmartWalletContext {
    ...
    createBurnerWallet: (scope: PublicKey, initialLamports: number) => Promise<void>,
    createSigner: (signer: PublicKey, scope: PublicKey) => Promise<void>,
    fillBurnerWallet: (burnerWalletKey: PublicKey, lamports: number) => Promise<void>
}
export const SmartWalletContext = React.createContext<ISmartWalletContext>({} as any);
export const useSmartWallet = () => useContext(SmartWalletContext)

const BURNER_WALLET_STORAGE_KEY = "burner_wallet";
export const SmartWalletProvider = ({ children }: { children: JSX.Element }) => {
    const { publicKey, sendTransaction } = useWallet();
    const { connection } = useConnection();
    const [burnerWalletStored, setBurnerWalletStored] = useLocalStorage<string | undefined>(BURNER_WALLET_STORAGE_KEY, undefined)
    const [burnerWallet, setBurnerWallet] = React.useState<Keypair>();
    const [burnerWalletBalance, setBurnerWalletBalance] = React.useState<number>(0);

    const [capabilities, setCapabilities] = React.useState<AccountInfoDeserialized<SignForMeAccount>[]>();
    const [delegatedSigners, setDelegatedSigners] = React.useState<AccountInfoDeserialized<SignForMeAccount>[]>();
    ...
    const reloadBurnerBalance = useCallback(async () => {
        if (burnerWallet) {
            ...
            const balance = await connection.getBalance(burnerWallet.publicKey, 'singleGossip');
    ...
    const selectionMemo = React.useMemo(
        () => ({
            ...
            createBurnerWallet: async (scope: PublicKey, initialLamports: number) => {
                if (!!burnerWalletStored)
                    throw new Error("Wallet already exist: " + burnerWallet?.publicKey?.toString());
                if (!scope)
                ...
                let [transactionSignForMe, _signForMe] = await createSignForMe(publicKey, key.publicKey, scope, publicKey);
            ...
            fillBurnerWallet: async (burnerWalletKey: PublicKey, lamports: number) => {
                if (lamports <= 0) {
                    return;
                }
                console.log(delegatedSigners);
                if (!delegatedSigners?.find((d) => !d.pubkey.equals(burnerWalletKey))) {
                    throw new Error("This burner wallet is not associated with your wallet")
                }

                await sendTransaction(new Transaction().add(SystemProgram.transfer({
                    fromPubkey: publicKey,
            ...
            createSigner: async (signer: PublicKey, scope: PublicKey) => {
                setLoading(true);
                let [transaction, _signForMe] = await createSignForMe(publicKey, signer, scope, publicKey);
        ...
        [network.config.type, publicKey, burnerWalletBalance, delegatedSigners, capabilities, loading, session, burnerWalletStored]
    );
