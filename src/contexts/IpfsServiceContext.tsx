...
export const encryptConfig = (config: IpfsServiceConfig, password: string): IpfsServiceConfig => {
    return cryptConfig(config, (string: string) => new Encrypter(password).encrypt(string))
}
...
export const decryptConfig = (config: IpfsServiceConfig, password?: string): Promise<IpfsServiceConfig> => {
    ...
    return hashPromise.then((hashedPassword) => {
        ...
        return password ? cryptConfig(config, (any: any) => new Encrypter(password).decrypt(any)) : config
    });
}
...
export interface IpfsContextState {
    ...
    getAdapter(password?: string, overrideServiceConfig?: IpfsServiceConfig): Promise<IPFSAdapter<any> | undefined>
...
export const IpfsServiceProvider: FC = ({ children, ...props }) => {
    ...
    const state = useMemo<IpfsContextState>(() => ({
        ...
        getAdapter: async (password?: string, overrideServiceConfig?: IpfsServiceConfig) => {
            if (!serviceConfig && !overrideServiceConfig)
                return undefined;
            if (overrideServiceConfig)
                return getAdapter(overrideServiceConfig)
